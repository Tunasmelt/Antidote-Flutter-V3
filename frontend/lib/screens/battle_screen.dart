...
class _BattleScreenState extends ConsumerState<BattleScreen> 
  ...
  void _startBattle() {
    ...
    final isValidUrl1 = url1.contains('spotify.com') || url1.contains('spotify:') || url1.contains('apple.com');
    final isValidUrl2 = url2.contains('spotify.com') || url2.contains('spotify:') || url2.contains('apple.com');
    
    if (!isValidUrl1 || !isValidUrl2) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please enter valid Spotify or Apple Music playlist URLs'),
          backgroundColor: AppTheme.cardBackground,
        ),
      );
      return;
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Icon(
              isTokenError ? Icons.music_off : Icons.error_outline,
              size: 64,
              color: isTokenError ? AppTheme.secondary : Colors.redAccent,
            ),
            const SizedBox(height: 24),
            Text(
              isTokenError ? 'Spotify Connection Required' : 'Battle Failed',
              style: Theme.of(context).textTheme.headlineSmall?.copyWith(
                color: AppTheme.textPrimary,
              ),
            ),
            ...
            Text(
              isTokenError 
                ? 'Please connect your Spotify account to battle playlists. Go to Settings to connect.'
            ...
            Row(
              ...
              children: [
                ...
                if (isTokenError) const SizedBox(width: 12),
                if (isTokenError)
                  OutlinedButton(
                    onPressed: () {
                      _compatibilityTimer?.cancel();
