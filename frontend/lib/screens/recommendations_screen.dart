...
class _RecommendationsScreenState extends ConsumerState<RecommendationsScreen> {
  ...
  Widget _buildTrackCard(Map<String, dynamic> track, int index) {
    ...
    // Handle Spotify track format: artists is an array
    String artist = 'Unknown Artist';
    if (track['artist'] != null) {
      artist = track['artist'].toString();
    } else if (track['artists'] is List && (track['artists'] as List).isNotEmpty) {
      final artistsList = track['artists'] as List;
      if (artistsList[0] is Map && artistsList[0]['name'] != null) {
        artist = artistsList[0]['name'].toString();
      } else {
    ...
    final albumArt = track['albumArt'] ?? 
        track['album_art'] ?? 
        (track['album'] is Map && track['album']['images'] is List && (track['album']['images'] as List).isNotEmpty
          ? (track['album']['images'] as List)[0]['url']
          : null) ??
        (track['images'] is List && (track['images'] as List).isNotEmpty
          ? (track['images'] as List)[0]['url']
          : null) ??
        track['image'];
